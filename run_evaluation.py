...
def run_pcw_experiment(datasets: List[str], models: List[str], cache_dir: str, subsample_test_set: int, output_dir: str,
                       ...
                       half_seed=None, window_shuffle_seed=None, use_retrieval=False, sort_by_label = False, 
                       block_select_method=None, block_order_method=None, block_group_method=None, n_selected_blocks=None, 
                       attn_implementation=None, attn_prev_blocks=None, attn_sink_blocks=None) -> None:
    ...
    for model in models:
        ...
        for dataset in datasets: 
            ...
            if window_shuffle_seed:
                ...
                print(f"New dataset name: {clean_dataset_name}")
            if use_retrieval:
                # no cache. retriev from context, or train_df
            ...
            if block_group_method:
                clean_dataset_name = f"{clean_dataset_name}-block_group={block_group_method}"
            if n_selected_blocks:
                clean_dataset_name = f"{clean_dataset_name}-n_selected_blocks={n_selected_blocks}"
            if block_select_method:
                clean_dataset_name = f"{clean_dataset_name}-block_select={block_select_method}"
            if block_order_method:
                clean_dataset_name = f"{clean_dataset_name}-block_order={block_order_method}"
            ...
            for nspw in n_shots_per_window:
                ...
                if pcw_model is None:
                    ...
                    elif (not pcw_base_model and len(n_windows) == 1 and n_windows[0] == 1) or block_select_method is not None:
                        if 'llama' in model.lower():
                            import torch
                            pcw_model = LlamaForCausalLM.from_pretrained(model, 
                ...
                # TODO we can add similar logic for n_windows if it is -1, choose the maximum possible
                ...
                em = ExperimentManager(test_df, train_df, pcw_model, tokenizer=tokenizer, labels=labels, random_seed=random_seed,
                                    ...
                                    block_select_method=block_select_method, block_order_method=block_order_method, block_group_method=block_group_method, 
                                    n_selected_blocks=n_selected_blocks, attn_prev_blocks=attn_prev_blocks, attn_sink_blocks=attn_sink_blocks)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-retrieval', help="apply retrieval method",
                        action='store_true', default=False)
    # new args
    parser.add_argument('--attn-implementation', help="attn_implementation, flex_attention / sdpa / flash_attention_2",
                        type=str, default="sdpa")
    parser.add_argument('--block-select-method', help="block selection method, all / random / bm25 / dense",
                        type=str, default=None)
    parser.add_argument('--block-order-method', help="block ordering method; hi2lo / lo2hi / mid2lo2high",
                        type=str, default=None)
    parser.add_argument('--block-group-method', help="block grouping method; default / balanced(-*) / bm25-clustering(-*) / embed-clustering(-*)",
                        type=str, default=None)
    parser.add_argument('--n-selected-blocks', help="number of blocks to select",
                        type=int, default=None)
    parser.add_argument('--attn-prev-blocks', help="number of local previous blocks attended to during encoding",
                        type=int, default=0)
    parser.add_argument('--attn-sink-blocks', help="number of sink(anchor) blocks attended to during encoding; -1 means full attention; will be interpret as attn-sink-tokens if out of bounds of number of block",
                        type=int, default=-1)
