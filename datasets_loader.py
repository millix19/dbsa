    ...
    def _load_dataset(self, test_from_train=False, prefer_val=False):
        if self.local:
            ...
            if self.subset is not None:
                dataset = load_dataset(self.dataset, self.subset, use_auth_token=self.token, ignore_verifications=True)
            else:  
                dataset = load_dataset(self.dataset, use_auth_token=self.token, ignore_verifications=True)
