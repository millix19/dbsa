...
def extract_intent(query: str) -> str:
    # Split the query by newline and colon
    parts = query.split('\n')
    for part in parts:
        if part.startswith('intent:'):
            # Extract the intent part after 'intent:'
            return part.split('intent:')[1].strip()
    return None
...
def retrieve_context_ablations(train_df: pd.DatetimeIndex, index: SparseRetriever, curr_example: str, n_examples: int, split_text, shuffle_seed=None):
    # ensure that logic is same as cached retrieval (in-order, include first block)
    retrieved = index.search(
        query=curr_example,    # What to search for        
        return_docs=False,          # Default value, return the text of the documents
        cutoff=n_examples,                # Default value, number of results to return
    ...
    if 0 in inds:
        ...
        inds.pop()
    inds = sorted(inds)
    dps = [train_df.loc[train_df['id'] == ind]['prompts'].values[0] for ind in inds]
    text = split_text.join(dps)
    return text

# new function that is same as retrieve_context but returns indices instead of text
def retrieve_context_indices(train_df: pd.DatetimeIndex, index: SparseRetriever, curr_example: str, n_examples: int, shuffle_seed=None):
    retrieved = index.search(
        query=curr_example,    # What to search for        
        return_docs=False,          # Default value, return the text of the documents
        cutoff=n_examples,                # Default value, number of results to return
    ...
    if len(inds) < n_examples:
        print(f"WARNING: sampling {n_examples - len(inds)} examples randomly to fill window")
        inds.extend(train_df['id'].sample(n_examples - len(inds)))
    # dps = list(train_df.loc[train_df['id'].isin(inds)]['text'])
    return inds


def create_retriever(train_df, dense=False):
    if dense:
        sr = DenseRetriever(
            index_name="training-examples",
            model="sentence-transformers/all-MiniLM-L12-v2",  # A lightweight and fast sentence transformer model
            normalize=True,
            max_length=512,
            use_ann=False,
        )
    ...
    filename = f"__temp_index_file_{os.getpid()}_{random.randint(1, 5888)}_{random.randint(1, 5999)}.csv"
...
# new function puts all examples in the index using create_retriever
def create_example_retriever(contexts):
    """
    Creates a retriever index for individual examples in contexts. contexts is a list of strings, where each string
    contains multiple examples separated by TEXT_BETWEEN_SHOTS.
    """
    examples = []
    for context in contexts:
        examples.extend(context.split(TEXT_BETWEEN_SHOTS))  # Split contexts into individual examples

    # Assign unique IDs to examples
    example_df = pd.DataFrame({"id": range(len(examples)), "text": examples, "prompts": examples})

    sr = create_retriever(example_df)
    return sr, example_df
...
def save_results(dataset: str, n_shots: List[int], results: np.ndarray[int], predictions: List[str], outpath: str,
    ...
    if not dist.is_initialized() or dist.get_rank() == 0:
        ...
        # with open(outpath.split(".")[-2] + "-outputs.pkl", 'wb') as f: # TODO should be outpath.split(".")[0] + outpath.split(".")[1]
        #     import pickle
        #     pickle.dump(predictions, f)
        clean_name = outpath.split(".")[-2].split('/')[-1] # example outpath = './test/togethercomputer+LLaMA-2-7B-32K/banking77-bm25/nspw=50_n_shots_results_800.npy'
